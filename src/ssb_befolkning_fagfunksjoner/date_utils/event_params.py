...
class EventParams:
    ...
    def __init__(
        ...
        year, period_type, period_number = self._prompt_missing_values(year, period_type, period_number)
    ...
    def _prompt_missing_values(
        ...
        if period_type == "year":
            ...
            if period_type == "halfyear":
                period_number = cls._prompt_int_in_range("Enter halfyear number", (1,2))
    ...
    @staticmethod
    def _prompt_period_type(msg: str, valid_choices: tuple[PeriodType, ...]) -> PeriodType:
        """
        ...
        """
        abbreviations: dict[str, PeriodType] = {c[0]: c for c in valid_choices}  # e.g. {"m": "month", "q": "quarter"}
        ...
        while True:
            ...
            print(f"'{value}' is not a valid option. Please choose one of: {valid_choices_str}.")
    ...
    def window(self) -> tuple[date, date]:
        ...
        if pn is None:
            raise ValueError("'period_number' is not set. Cannot derive window for non-year periods.")
        ...
        raise ValueError(f"{self.period_type} is not a valid option. Please choose one of: {self.VALID_PERIOD_TYPES}.")
    ...
    def _add_wait_period(d: date, months: int, days: int, *, boundary: str) -> date:
        ...
        if boundary == "start":
            ...
            last_day_of_month = calendar.monthrange(added_months.year, added_months.month)[1]
            return added_months.replace(day=last_day_of_month) + relativedelta(days=days)
    ...
    def etterslep_window(self) -> tuple[date, date]:
        ...
        etterslep_start = self._add_wait_period(start, self.wait_months, self.wait_days, boundary="start")
        etterslep_end = self._add_wait_period(end, self.wait_months, self.wait_days, boundary="end")
