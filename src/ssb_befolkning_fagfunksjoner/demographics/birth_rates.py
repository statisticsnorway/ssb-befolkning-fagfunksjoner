...
class BirthRates:
    ...
    def _beregn_middelfolkemengde(
        ...
        a = df_start.groupby(grupperingsvariabler, dropna=False).size().reset_index(name="n_df_start")
        b = df_slutt.groupby(grupperingsvariabler, dropna=False).size().reset_index(name="n_df_slutt")
