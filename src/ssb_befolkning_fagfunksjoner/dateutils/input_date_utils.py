...
def prompt_for_int(prompt_msg: str, valid_range: tuple[int, int] | None = None) -> int:
    """
...
def get_user_inputs(specify_wait_period: bool = False) -> tuple[int, str, int|None, int, int]:
    ...
    while True:
        period_type = input("Enter period type (year/halfyear/quarter/month/week): ").strip()
        if period_type in VALID_PERIOD_TYPES:
            break
        else:
            print("Invalid period type. Choose from year, halfyear, quarter, month, week.")

    if period_type == "year":
        period_number = None
    elif period_type == "halfyear":
        period_number = prompt_for_int("Enter halfyear (1-2): ", (1, 2))
    elif period_type == "quarter":
        period_number = prompt_for_int("Enter quarter (1-4): ", (1, 4))
    elif period_type == "month":
        period_number = prompt_for_int("Enter month (1-12): ", (1,12))
    elif period_type == "week":
        period_number = prompt_for_int("Enter week (1-53): ", (1, 53))
