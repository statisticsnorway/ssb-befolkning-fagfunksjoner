...
@patch("src.ssb_befolkning_fagfunksjoner.versions.get_fileversions")
def test_write_versioned_pandas(
    ...
    mock_write_pandas.assert_any_call(
        df=df, gcs_path="gs://bucket/folder/file_v2.parquet"
    )
    ...
    mock_fs_mv.assert_called_once_with(
        "gs://bucket/folder/file.parquet", "gs://bucket/folder/file_v1.parquet"
    )
...
def test_write_versioned_pandas_no_existing_files(
    ...
    mock_write_pandas.assert_called_once_with(
        df=df, gcs_path="gs://bucket/folder/file.parquet"
    )
...
def test_write_versioned_pandas_several_existing_files(
    ...
    mock_write_pandas.assert_any_call(
        df=df, gcs_path="gs://bucket/folder/file_v3.parquet"
    )
    mock_write_pandas.assert_any_call(df=df, gcs_path="gs://bucket/folder/file.parquet")
