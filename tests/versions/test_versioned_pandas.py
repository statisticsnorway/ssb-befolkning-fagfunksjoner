...
@pytest.fixture
def patched_env():
    with mock.patch("ssb_befolkning_fagfunksjoner.versions.UPath") as mock_upath, \
        mock.patch("ssb_befolkning_fagfunksjoner.versions.get_next_version_number") as mock_get_next_version_number, \
        mock.patch("ssb_befolkning_fagfunksjoner.versions.get_fileversions") as mock_get_fileversions:
...
def test_second_write_promotes_and_versions(dummy_df, patched_env, patched_to_parquet, patched_fs):
    mock_upath, mock_get_version, mock_get_versions = patched_env
    mock_path = mock.Mock()
    ...
    patched_fs.copy.assert_called_once_with("gs://mock/parent/testfile.parquet", "gs://mock/parent/testfile_v1.parquet")
...
def test_input_with_version_suffix_raises(dummy_df, patched_env):
    ...
    with pytest.raises(ValueError, match="Detected versioning in function parameter"):
        write_versioned_pandas(dummy_df, "gs://bucket/testfile_v2.parquet", overwrite=False)
